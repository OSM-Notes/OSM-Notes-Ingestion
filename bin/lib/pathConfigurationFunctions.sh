...
function __init_log_dir() {
 ...
 if [[ "${BASENAME}" == *"Daemon"* ]] || [[ "${BASENAME}" == *"daemon"* ]]; then
  SCRIPT_TYPE="daemon"
 elif [[ "${BASENAME}" == *"Monitor"* ]] || [[ "${BASENAME}" == *"monitor"* ]] ||
     [[ "${BASENAME}" == *"Check"* ]] || [[ "${BASENAME}" == *"check"* ]]; then
...
function __init_tmp_dir() {
 ...
 local TMP_DIR
 TMP_DIR=$(mktemp -d "${BASE_TMP_DIR}/${BASENAME}_XXXXXX" 2> /dev/null || \
           mktemp -d "/tmp/${BASENAME}_XXXXXX")
...
function __init_directories() {
 ...
 if [[ -z "${BASENAME}" ]]; then
  BASENAME=$(basename -s .sh "${0:-unknown}" 2> /dev/null || echo "unknown")
 fi
 ...
 local LOG_DIR_VAL
 LOG_DIR_VAL=$(__init_log_dir "${BASENAME}" "${FORCE_FALLBACK}")
 local TMP_DIR_VAL
 TMP_DIR_VAL=$(__init_tmp_dir "${BASENAME}" "${FORCE_FALLBACK}")
 local LOCK_DIR_VAL
 ...
 # Set log filename
 export LOG_FILENAME="${LOG_DIR}/${BASENAME}.log"

 # Set lock file path
 export LOCK="${LOCK_DIR}/${BASENAME}.lock"
