...
### Ignoring Local Changes in Production

When deploying to a production server, you may need to modify properties files
with environment-specific values (database credentials, server URLs, etc.).
To prevent Git from asking you to commit these production-specific changes,
you can use Git's `skip-worktree` feature.

#### Quick Setup

```bash
# Enable skip-worktree for all properties files
bin/scripts/manageProductionProperties.sh enable

# Check current status
bin/scripts/manageProductionProperties.sh status

# Disable skip-worktree (before pulling updates)
bin/scripts/manageProductionProperties.sh disable
```
...
```bash
# Enable skip-worktree for specific files
git update-index --skip-worktree etc/properties.sh
git update-index --skip-worktree etc/wms.properties.sh

# Check status
git ls-files -v | grep "^S"

# Disable skip-worktree (before pulling updates)
git update-index --no-skip-worktree etc/properties.sh
git update-index --no-skip-worktree etc/wms.properties.sh
```

#### Important Notes

- **Before pulling updates**: Always disable skip-worktree first, then pull,
  then re-enable it. This ensures you get the latest changes from the repository.

- **Workflow**:
  1. `manageProductionProperties.sh disable`
  2. `git pull` (or `git merge`)
  3. Resolve any conflicts if needed
  4. `manageProductionProperties.sh enable`
  5. Apply your production-specific changes

- **What skip-worktree does**: It tells Git to ignore local changes to tracked
  files. The files remain in the repository, but Git won't show them as modified
  even if you change them locally.

- **Alternative approach**: Consider using environment variables or a separate
  `properties.sh_local` file that is loaded after `properties.sh` and is
