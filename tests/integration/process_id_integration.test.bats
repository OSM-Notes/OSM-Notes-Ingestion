...
@test "PROCESS_ID should be VARCHAR for locks but INTEGER for app.process_id" {
 ...
 # Step 5: Set app.process_id in PostgreSQL and verify it's INTEGER in the same session
 local result
 result=$(psql -d "${DBNAME}" -Atq 2>/dev/null << EOF
SET app.process_id = '${PROCESS_ID_INTEGER}';
SELECT current_setting('app.process_id', true)::INTEGER;
EOF
) || result="0"
 
 # Result should be a number matching PROCESS_ID_INTEGER
 [[ "${result}" =~ ^[0-9]+$ ]]
 [ "${result}" -eq "${PROCESS_ID_INTEGER}" ]
}

@test "app.process_id should fail if set with underscores" {
 local invalid_process_id="12345_67890_11111"
 
 # Attempt to set app.process_id with underscores and convert to INTEGER in same session
 # This should fail because underscores cannot be converted to INTEGER
 run psql -d "${DBNAME}" -Atq 2>&1 << EOF
SET app.process_id = '${invalid_process_id}';
SELECT current_setting('app.process_id', true)::INTEGER;
EOF
...
EOF
 ...
 # Step 4: Set app.process_id and execute SQL that uses it in the same session
 local result
 result=$(psql -d "${DBNAME}" 2>&1 << EOF
SET app.process_id = '${PROCESS_ID_INTEGER}';
DO \$\$
DECLARE
...
EOF
 ...
 [[ ! "${result}" == *"ERROR"* ]]
 [[ "${result}" == *"process_id:"* ]]
 # Verify the process_id value matches (extract from NOTICE)
 [[ "${result}" == *"process_id: ${PROCESS_ID_INTEGER}"* ]]
}

@test "SQL insertion should use INTEGER process_id from app.process_id" {
 # Step 1: Create test tables with unique names to avoid conflicts
 psql -d "${DBNAME}" << 'EOF' > /dev/null 2>&1 || true
DROP TABLE IF EXISTS test_notes_api_part_1 CASCADE;
DROP TABLE IF EXISTS test_notes_api CASCADE;
DROP TABLE IF EXISTS test_notes CASCADE;
CREATE TABLE test_notes_api (
 note_id INTEGER NOT NULL,
 latitude DECIMAL NOT NULL,
 longitude DECIMAL NOT NULL,
 created_at TIMESTAMP NOT NULL,
...
CREATE TABLE test_notes_api_part_1 PARTITION OF test_notes_api FOR VALUES FROM (1) TO (2);
...
EOF
 ...
 # Step 2: Insert test data into test_notes_api
 psql -d "${DBNAME}" << 'EOF' > /dev/null 2>&1
INSERT INTO test_notes_api (note_id, latitude, longitude, created_at, closed_at, status, id_country, part_id)
VALUES (999, 40.7128, -74.0060, '2023-01-01T00:00:00Z', NULL, 'open', NULL, 1)
ON CONFLICT DO NOTHING;
EOF
 
 # Step 3: Set app.process_id as INTEGER and execute SQL in the same session
 local PROCESS_ID_INTEGER="${$}"
 local result
 result=$(psql -d "${DBNAME}" 2>&1 << EOF
SET app.process_id = '${PROCESS_ID_INTEGER}';
...
BEGIN
 ...
 LOOP
  ...
  INSERT INTO test_notes (id, note_id, lat, lon, created_at, closed_at, status, id_country)
  VALUES (r.note_id, r.note_id, r.latitude, r.longitude, r.created_at, r.closed_at, r.status, NULL)
  ON CONFLICT (id, note_id) DO NOTHING;
...
EOF
 ...
 # Should succeed without "invalid input syntax" error
 [[ ! "${result}" == *"invalid input syntax"* ]]
 # Check for actual errors (not NOTICE messages that might contain "ERROR" text)
 if echo "${result}" | grep -qiE "^ERROR|^FATAL|^PANIC"; then
  echo "Actual error detected: ${result}"
  false
 fi
 [[ "${result}" == *"Inserted note"* ]]
 # Verify the process_id value matches
 [[ "${result}" == *"process_id ${PROCESS_ID_INTEGER}"* ]]
 
 # Step 5: Verify note was inserted
 local count
 count=$(psql -d "${DBNAME}" -Atq -c "SELECT COUNT(*) FROM test_notes WHERE note_id = 999;" 2>/dev/null || echo "0")
 [ "${count}" -eq "1" ]
